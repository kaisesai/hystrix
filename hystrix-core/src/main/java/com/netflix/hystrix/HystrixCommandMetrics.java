...
public class HystrixCommandMetrics extends HystrixMetrics {
        ...
        switch (eventType) {
          case EXCEPTION_THROWN:
            for (HystrixEventType exceptionEventType : HystrixEventType.EXCEPTION_PRODUCING_EVENT_TYPES) {
              cumulativeEvents[eventType.ordinal()] += bucketEventCounts[exceptionEventType
                .ordinal()];
            }
            break;
          default:
  ...
  private static final ConcurrentHashMap<String, HystrixCommandMetrics> metrics
    = new ConcurrentHashMap<String, HystrixCommandMetrics>();
    ...
    if (commandMetrics != null) {
      ...
      synchronized (HystrixCommandMetrics.class) {
        ...
        if (existingMetrics != null) {
          ...
          HystrixCommandMetrics newCommandMetrics = new HystrixCommandMetrics(key, commandGroup,
  ...
  private final RollingCommandUserLatencyDistributionStream
  ...
  /* package */HystrixCommandMetrics(final HystrixCommandKey key,
                                     HystrixCommandGroupKey commandGroup,
                                     HystrixThreadPoolKey threadPoolKey,
                                     HystrixCommandProperties properties,
  ...
  /* package-private */ void markCommandStart(HystrixCommandKey commandKey,
                                              HystrixThreadPoolKey threadPoolKey,
    ...
    HystrixThreadEventStream.getInstance()
                            .commandExecutionStarted(commandKey, threadPoolKey, isolationStrategy,
                                                     currentCount);
  }

  /* package-private */ void markCommandDone(ExecutionResult executionResult,
  ...
  public static class HealthCounts {
    ...
    public HealthCounts plus(long[] eventTypeCounts) {
      ...
      long threadPoolRejectedCount = eventTypeCounts[HystrixEventType.THREAD_POOL_REJECTED
        .ordinal()];
      long semaphoreRejectedCount = eventTypeCounts[HystrixEventType.SEMAPHORE_REJECTED.ordinal()];

      updatedTotalCount += (successCount + failureCount + timeoutCount + threadPoolRejectedCount
        + semaphoreRejectedCount);
      updatedErrorCount += (failureCount + timeoutCount + threadPoolRejectedCount
        + semaphoreRejectedCount);
